...
8. **QC/レビュー時のスキル起動**: 品質チェック・レビュー・共有前は `sp-verification-before-completion` と `analysis-qa-checklist` の SKILL.md を読んでから実施。 **レポートは更新時も冒頭を更新する**——末尾に章を追加して終えない。更新で見つかった事項は「重要な漏れ・改善余地」であることが多く、最も手前に来るべきもの。冒頭の20%で全体の価値の80%を説明できる構成に、初回作成時も更新時も常に保つ。機械ガード: `.claude/hooks/summary_freshness_guard.py`（Stop・冒頭ブロックが逐語で不変のまま後半だけ変更されたターンを警告）。**初回作成時も冒頭に結論節を置く**（新規レポートに結論/サマリー節が無い、または冒頭が各章に触れていない場合、`pre_report_quality_guard.py` が書き込み時に警告する。既存ファイルの負債では発火せず、その編集で悪化した場合のみ。※機械が見るのは「結論節の有無」と「冒頭が各章に触れているか」までで、要点が実際に書けているかは判定していない）。
