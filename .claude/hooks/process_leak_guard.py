...
# 「撤回ルール」「点検手順」のように *規則の名前* が同じ行にある場合、その行は
# 規則の定義であって自分の制作過程の報告ではない（2026-09-19 実測: 撤回 54 件中
# 該当 1 件＝facility-search/RESEARCH_QUALITY_IMPROVEMENT_20260702.md L51。
# 残り 53 件の本物の撤回報告は一切抑止されないことを確認済み）。
RULE_NAME = re.compile(
    u"(撤回|訂正|検証|点検|レビュー|確認)(ルール|規則|手順|方針|基準|フロー|ゲート)")
RULE_NAME_LABELS = (u"自分の主張の撤回", u"第三者確認の露出")
...
# 設計・運用の判断そのものを論じる文書では、スクリプト名やフック名は「制作過程の
# 露出」ではなく **判断の対象** である。どのファイルを正典にしたかが結論だから、
# 付録へ移設すると結論の根拠が読めなくなる（2026-09-19、
# RESEARCH_REPOS_CONSOLIDATION_20260914.md で実測: 本文に結論の要点表を足した
# だけで既存本文の 12 行が発火した。中身は表の区切り行と verify_citations.py 等の
# ファイル構成で、統合判断レポートの中核だった）。
# SOURCE_DOC と同じく **ツール名ラベルにのみ** 適用し、是正件数の自己申告・
# 点検ラウンド・品質宣言といった本物の制作過程クラスは一切免除しない。
# 判定は H1 タイトルに対して行う。本文に「フック」が一度出る文書は山ほどあるが、
# タイトルがツール／構成／運用そのものを名指す文書は、その話題が主題である。
ENGINEERING_DOC = re.compile(
    u"^#\\s.*?(リポジトリ|フック|hook|ガード|guard|正典|配布|デプロイ|"
    u"マニフェスト|manifest|アーキテクチャ|構成|統合|較正|calibration|"
    u"ゲート|gate|スクリプト|ツール|運用ルール|プロセス|"
    u"スキル|skill|品質改善|品質チェック|セットアップ|設計書)",
    re.I | re.M)
...
PROCESS_LEAK = [
    ...
    (SELF + NEG + r"[^。\n\x0b]{0,40}" + ACT +
     r"[^。\n\x0b]{0,10}(済み|した|しています|を経て|を通|完了|通過|合格)",
     "自作物の検証工程の開示"),
    # --- 版履歴・前版の主張への言及（ユーザー指摘の中核クラス） ---
    # 較正（2026-09-09/10 実測・Soulful-Content STRATEGY_V2）: `v\d` と「評価|修正|改め|誤」を
    # 含めたところ「v2 案の議論・評価」「v2 案は骨格を採用」など**提案の名前**を版履歴と誤検出した。
    # 版履歴と判定するのは、前版を指す語＋自作物の主張を指す語の共起のみに限る。
    (r"(初版|旧版|前版|前回|当初|初出|v\d)[^。\n\x0b]{0,40}"
     r"(撤回|改め|訂正|誤|主張|判定|見立て)", "自分の前版への言及"),
    # 「撤回ルール」「撤回の手順」等は *規則の名前* であって自分の撤回ではない。
    # 直前に規則・手順を指す語が来る形だけを除く（2026-09-19 実測: 全リポ 52 件中
    # この形は facility-search/RESEARCH_QUALITY_IMPROVEMENT_20260702.md の 1 件のみ。
    # 残り 51 件は本物の撤回報告で、そのまま発火する）。
    ...
    # 「独立レビュー」は語そのものが制作過程の自慢ではない。運用ルール・スキル定義・
    # モデル使い分け表では *工程の名前* として出るだけで、実測 419 件中 378 件が
    # それだった（2026-09-19、全リポ .md 走査）。露出と呼べるのは「この文書が独立
    # 確認を受けた／その指摘で直した」という**完了の報告**に限る。完了形を必須に
    # した結果 419→41 件。本物（HARD_WORK_MENTAL_HEALTH_20260830.md L982/L1245 の
    # 「独立レビューの指摘で追加」等）は全数残ることを確認済み。
    (r"(第三者|独立)(の目|レビュ[アーー]*|確認)[^。\n\x0b]{0,20}"
     r"(を(経|受け|通|クリア|パス)|済み|実施(済|した|しました)|"
     r"にかけ(た|ました)|で(検証|確認)(済|した|しました)|"
     r"(を|の|で|から)[^。\n\x0b]{0,10}"
     r"(指摘|助言|反映|踏まえ|のち|の後|もとに|基づ|によ(り|って)|"
     r"判明|発覚|訂正|撤回))", "第三者確認の露出"),
...
def _scan_lines(text):
    ...
    for i, line in enumerate(raw):
        ...
        # セル境界 | は垂直タブ（U+000B）に置換する。\s には一致するので「| 是正 | 3件 |」の
        # 分断攻撃は従来どおり捕まえ、SELF…ACT の長いギャップ [^。改行 U+000B] は越えられない。
        # 空白に潰していたときは「| Claude Code | §3.1 の API は動作確認済み」のように
        # 別々の列の語が1文として一致した（2026-09-09 実測・誤検出）。
        out.append((i + 1, line.replace("|", "\x0b"), joined.replace("|", "\x0b")))
    return out
...
def main():
    ...
    for tgt in targets:
        ...
        is_source_doc = bool(SOURCE_DOC.search(text)
                             or ENGINEERING_DOC.search(text))
        ...
        for lineno, line, joined in rows:
            ...
            for pat, label in PROCESS_LEAK:
                ...
                if label in RULE_NAME_LABELS and RULE_NAME.search(hit_text):
                    continue
