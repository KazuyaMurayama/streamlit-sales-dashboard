...
def _prompt(tier, request, files, answer):
    ...
    if tier == 1:
        return ("【自動レビュー（1回のみ）】以下のユーザー依頼に対し、回答が"
                ...
                "点検は思考内で行う。問題なければ何も追記せずに終える。"
                "不足を補った場合のみ、その内容だけを追記する。\n---\n" + req)
    ...
    body = ("【自動レビュー（1回のみ・Tier2）】下記はユーザーの依頼原文である。"
            ...
            "点検は思考内で行い、対応表・引用は回答に書かない"
            "（表示すると回答が2本になり長くなる。実測 +24%）。"
            "不足があればこのターンで補い、補った内容だけを追記する。問題なければ追記しない。"
            "未回答の項目は「未回答: 〜」と1行で明記し、閾値を下げて充足扱いにしない。"
